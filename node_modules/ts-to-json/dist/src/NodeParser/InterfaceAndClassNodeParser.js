...
const extendKey_1 = require("../Utils/extendKey");
const isExcludedProp_1 = require("../Utils/isExcludedProp");
...
class InterfaceAndClassNodeParser {
    constructor(typeChecker, childNodeParser, additionalProperties, config) {
    ...
    getProperties(node, context) {
        ...
        const properties = node.members
            .filter((member) => !(0, isExcludedProp_1.isExcludedProp)(member, context, this.config))
            ...
            .map((member) => new ObjectType_1.ObjectProperty(this.getPropertyName(member.name), this.childNodeParser.createType(typescript_1.default.isMethodSignature(member) ? member : member.type, context), !member.questionToken))
            .filter((prop) => {
    ...
    getTypeId(node, context) {
        ...
        return (0, extendKey_1.extendKey)(`${nodeType}-${(0, nodeKey_1.getKey)(node, context)}`, node, context, this.config, true);
    }
