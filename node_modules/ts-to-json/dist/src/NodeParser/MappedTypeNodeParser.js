...
class MappedTypeNodeParser {
    constructor(childNodeParser, additionalProperties, config) {
    ...
    createType(node, context) {
        ...
        if (keyListType instanceof UnionType_1.UnionType) {
            ...
            throw new LogicError_1.LogicError(`Unexpected key type "${constraintType ? constraintType.getId() : constraintType}" for type "${node.getText()}" (expected "UnionType" or "StringType")`, node);
        }
    }
    ...
    getProperties(node, keyListType, context) {
        return keyListType
            ...
            .filter((type) => !(0, isExcludedProp_1.isExcludedProp)(type, new NodeParser_1.Context(node), this.config))
            .reduce((result, key) => {
    ...
    createSubContext(node, key, parentContext) {
        const subContext = new NodeParser_1.Context(node, parentContext);
